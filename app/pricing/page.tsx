...
export default function PricingPage() {
  ...
  const openMail = (optionLabel: string) => {
    const subject = t('pricing.emailSubject');

    // Convert literal \n from translation into real new lines
    const bodyTemplate = t('pricing.emailBody').replace(/\\n/g, '\r\n');
    const bodyRaw = bodyTemplate.replace('{{option}}', optionLabel);

    const mailto = `mailto:info@3comp.si?subject=${encodeURIComponent(
      subject
    )}&body=${encodeURIComponent(bodyRaw)}`;

    // ESLint-friendly (no direct href assignment)
